  ...
  <div id="sq-ccbox">
    ...
    <form id="nonce-form" novalidate action="path/to/payment/processing/page" method="post">
      ...
      <div id="sq-walletbox">
        ...
        <v-btn v-show=applePay :id="id+'-sq-apple-pay'" class="button-apple-pay"></v-btn>

        <div v-show=!masterpass class="wallet-not-enabled">Masterpass not enabled</div>
        <!-- Placeholder for Masterpass button -->
        <v-btn v-show=masterpass :id="id+'-sq-masterpass'" class="button-masterpass"></v-btn>
      </div>
    </form>
  </div>
  <v-btn @click="requestCardNonce($event)" class='productPurchase payButton'>Pay</v-btn>
</div>
</template>
...
export default {
  ...
  mounted: function() {
    let locationId = "CBASENp7T9f5KN5pEXWqA6F7IUogAQ";
    let applicationId = "sandbox-sq0idp-gzCuCDdgyfPKu2mHfQChGQ";
    let that = this;
