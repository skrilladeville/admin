    ...
    <v-layout row>
      ...
      <v-flex md6 pt-5 mr-3>
        <v-card height="400">
          ...
          <template v-if="has_prod_emp">
            ...
            <br>
            <br>
            <v-alert :value="true" color="info" icon="info" outline>No products found</v-alert>
          </template>
          ...
          <v-layout
            ...
            wrap
            id="style-5"
            style="height:330px; max-height:330px; overflow-y:scroll;"
          >
        ...
        <v-layout row pt-1 ml-1 mr-1 wrap>
          <v-flex sm3 pa-1>
            <v-card
              height="60"
              @click="setAll"
              style="background: linear-gradient(#26c6da, #90caf9);"
              hover
              raised
              class="rounded-card white--text"
            >
              <v-card-title>
                <span>all</span>
              </v-card-title>
      ...
      <v-flex md6 pt-5>
        <v-card height="540">
          <v-card-title>
            <el-input
              @keyup.enter.native="addProductbyBarcode()"
              v-model="barcode"
          ...
          <v-layout mr-2 ml-2 row wrap style="height:220px; max-height:220px; overflow-y:scroll;">
            <v-flex md12>
              ...
              >
                ...
                <el-table-column prop="product" label="product" width="160"></el-table-column>
                <el-table-column prop="qty" label="qty" width="80"></el-table-column>
          ...
          <v-layout ml-2 mr-2 row wrap>
            <v-flex ma-2 md12>
              <v-card height="150">
                <v-layout row wrap ml-3 pa-2>
                  ...
                  <v-flex md4>
                    <h6>{{(total-taxtotal).toFixed(2)}}</h6>
                  </v-flex>
                  <v-flex md8>
                    <h6>Discount</h6>
                  </v-flex>
            ...
            <v-flex md3>
              <v-btn
                ...
                block
                style="color:white !important;"
                @click="dialogCancelTrans=true"
              >F5-Void Transaction</v-btn>
            </v-flex>
            <v-flex md3>
              <v-btn
                color="error"
      ...
      <v-dialog persistent v-model="dialogAdd" max-width="500">
        <v-card>
          ...
          <v-card-actions>
            ...
            <v-btn color="green darken-1" flat="flat" @click="addProduct">add</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
    </v-layout>
            ...
            >
              ...
              <el-form-item label="Card Holder Name">
                <el-input
                  type="text"
                  id="cardholderName"
                  v-model="formcard.cardholdername"
                  autocomplete="off"
                ></el-input>
              </el-form-item>
      ...
      <v-dialog v-model="dialogVoidLine" persistent max-width="350">
        <v-card>
          ...
          <v-card-text>
            ...
            <el-input type="password" placeholder="password here"></el-input>
          </v-card-text>
          <v-card-actions>
            <v-spacer></v-spacer>
...
export default {
  ...
  mounted() {
    ...
    rpx.setAttribute('src', '/js/rpx.js')
    rpx.async = true
    document.head.appendChild(rpx)

    axios
      .get('/api/sales/transaction/show/' + this.$route.params.transid)
      .then(res => {
  ...
  created() {
    ...
    axios
      .get('/api/sales/order/show/' + this.$route.params.orderid)
      .then(res => {
        this.order = res.data
        console.log(res.data)

        axios
          .get('/api/sales/orderItem/byOrder/' + this.order.id)
          .then(res => {
  ...
  methods: {
    ...
    validate() {
      var cardNumberCheck = this.RealexRemote().validateCardNumber(
        document.getElementById('cardNumber').value
      )
      var cardHolderNameCheck = this.RealexRemote().validateCardHolderName(
        document.getElementById('cardholderName').value
      )
      var expiryDate = document
        .getElementById('expiryDateMM')
        .value.concat(document.getElementById('expiryDateYY').value)
      var expiryDateFormatCheck = this.RealexRemote().validateExpiryDateFormat(
        expiryDate
      )
      var expiryDatePastCheck = this.RealexRemote().validateExpiryDateNotInPast(
        expiryDate
      )
      if (document.getElementById('cardNumber').value.charAt(0) == '3') {
        var cvnCheck = this.RealexRemote().validateAmexCvn(
          document.getElementById('cvn').value
        )
      } else {
        var cvnCheck = this.RealexRemote().validateCvn(
          document.getElementById('cvn').value
        )
      }
      if (
        cardNumberCheck == false ||
        cardHolderNameCheck == false ||
        expiryDateFormatCheck == false ||
        expiryDatePastCheck == false ||
        cvnCheck == false
      ) {
        ...
        if (prod != null) {
          if (prod.product.price_measurement == 'Weight') {
            ...
            if (prod.size == '1g') {
              ...
              this.addProducts.push({
                product_id: p.id,
                product: `${prod.product.name} ${prod.size}`,
                price: p.prices[0].gram_price,
                qty: 1,
                amount: p.prices[0].gram_price
              })
            }
            if (prod.size == '1/8oz') {
              ...
              this.addProducts.push({
                product_id: p.id,
                product: `${prod.product.name} ${prod.size}`,
                price: p.prices[0].eight_price,
                qty: 1,
                amount: p.prices[0].eight_price
              })
            }
            if (prod.size == '1/4oz') {
              let p = this.products.find(el => el.id == prod.product.id)
              this.addProducts.push({
                product_id: p.id,
                product: `${prod.product.name} ${prod.size}`,
                price: p.prices[0].quarter_price,
                qty: 1,
                amount: p.prices[0].quarter_price
              })
            }
            if (prod.size == '1/2oz') {
              let p = this.products.find(el => el.id == prod.product.id)
              this.addProducts.push({
                product_id: p.id,
                product: `${prod.product.name} ${prod.size}`,
                price: p.prices[0].half_price,
                qty: 1,
                amount: p.prices[0].half_price
              })
            }
            ...
            if (prod.size == 'pre-roll') {
              ...
              this.addProducts.push({
                product_id: p.id,
                product: `${prod.product.name} ${prod.size}`,
                price: p.prices[0].joint_price,
                qty: 1,
                amount: p.prices[0].joint_price
              })
            }
          }
        }
          ...
          if (prod.product.price_measurement == 'Weight') {
            ...
            if (prod.size == '1g') {
              ...
              this.addProducts.push({
                ...
                price: p.prices[0].gram_price,
                qty: res[1],
                amount: p.prices[0].gram_price * res[1]
              })
            }
            if (prod.size == '1/8oz') {
              console.log('naka abot pud')
              ...
              this.addProducts.push({
                ...
                price: p.prices[0].eight_price,
                qty: res[1],
                amount: p.prices[0].eight_price * res[1]
              })
            }
            if (prod.size == '1/4oz') {
              let p = this.products.find(el => el.id == prod.product.id)
              this.addProducts.push({
                ...
                price: p.prices[0].quarter_price,
                qty: res[1],
                amount: p.prices[0].quarter_price * res[1]
              })
            }
            if (prod.size == '1/2oz') {
              let p = this.products.find(el => el.id == prod.product.id)
              this.addProducts.push({
                ...
                price: p.prices[0].half_price,
                qty: res[1],
                amount: p.prices[0].half_price * res[1]
              })
            }
            ...
            if (prod.size == '1oz') {
              ...
              this.addProducts.push({
                ...
                price: p.prices[0].half_price,
                qty: res[1],
                amount: p.prices[0].ounce_price * res[1]
              })
            }
            ...
            if (prod.size == 'pre-roll') {
              ...
              this.addProducts.push({
                ...
                price: p.prices[0].joint_price,
                qty: res[1],
                amount: p.prices[0].joint_price * res[1]
              })
            }
          }
        }
    ...
    voidline() {
      ...
      axios
        .post('/api/sales/orderItem/delete/' + this.currentRow.id)
        .then(res => {
          console.log(res)
    ...
    canceltrans() {
      axios
        .post('/api/sales/transaction/void/' + this.$route.params.transid)
        .then(res => {
          axios
            .post('/api/sales/order/void/' + this.$route.params.orderid)
            .then(res => {
              this.$router.push(
                '/pos/terminal/' + this.$route.params.terminalId
              )
    ...
    addProduct() {
      if (this.product.price_measurement == 'Weight') {
        ...
        axios
          .post('/api/sales/orderItem/create', {
            order_id: this.$route.params.id,
            product_name: prod,
            product_id: this.product.id,
            qty: this.count,
            price: this.price,
            tax: this.price * 0.1 * this.count
          })
          .then(el => {
            console.log(el.data)
            ...
            this.addProducts.push({
              ...
              product: el.data.product_name,
              price: el.data.price,
              qty: el.data.qty,
              amount: el.data.price * el.data.qty,
              tax: el.data.tax
            })
          })
      ...
      if (
        this.product.price_measurement == 'Per Unit' ||
        this.product.price_measurement == 'Per Unit Range'
      ) {
    ...
    changeCount(value) {
      if (this.product.price_measurement == 'Per Unit Range') {
        let max = Math.max.apply(
          Math,
          this.product.prices.map(function(p) {
            return p.count
          })
        )
        ...
        if (value < max) {
          this.product.prices.forEach(el => {
            if (value == el.count) {
              ...
              this.price = (
                this.product.prices[0].piece_price /
                this.product.prices[0].count
              ).toFixed(2)
            }
          })
        } else {
      ...
      if (value < max) {
        this.product.prices.forEach(el => {
          ...
          if (inRange) {
            ...
            this.price = (
              this.product.prices[0].range_price / this.product.prices[0].to
            ).toFixed(2)
          }
        })
      } else {
  ...
  computed: {
    ...
    products_computed() {
      let filter = this.products.filter(el =>
        el.name.toLowerCase().includes(this.product_query.toLowerCase())
      )
      let coutCatProd = 0
      if (
        (filter.length == 0 && this.product_query != '') ||
        coutCatProd.length == 0
      ) {
      ...
      if (this.product_query == '') {
        ...
        if (this.category_id != '') {
          coutCatProd = this.products.filter(
            el => el.product_category_id == this.category_id
          )
