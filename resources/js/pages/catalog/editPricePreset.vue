...
export default {
    created(){
        axios.get(`/api/catalog/pricePreset/view/${this.$route.params.id}`)
            .then(res=>{
                this.form=res.data
                this.form.prices=res.data.prices
...
form:{
    ...
    methods:{
        onSubmit(){
            axios.post(`/api/catalog/pricePreset/update/${this.$route.params.id}`,this.form)
                .then(res=>{
                    console.log(res)
