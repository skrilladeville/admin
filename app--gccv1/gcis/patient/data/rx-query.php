...
foreach( $fields as $colname )
	if( !in_array( $colname,array( 'lname','refill_qty' ) ) )
		$joinTableColNames[] = $sqlDB['table'].".".$colname;

// doctor_rx.user_id='".$user['id']."' AND 
$query = "SELECT doctor_accounts.lname,patient_feedback.refill_qty,doctor_diagnosis.fill_balance,". implode(',',$joinTableColNames) ." FROM ".$sqlDB['table']." 
LEFT JOIN doctor_rx ON doctor_diagnosis.rx_id=doctor_rx.id 
LEFT JOIN doctor_accounts ON doctor_rx.user_id=doctor_accounts.user_id 
