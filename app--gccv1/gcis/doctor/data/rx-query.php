...
$fields = array( 
	...
	'birthday', //patient_bday
	'id'
);

foreach( $fields as $colname )
	if( !in_array( $colname,array( 'id','buttons' ) ) )
		$joinTableColNames[] = 'patient_profiles.'.$colname;

$query = "SELECT doctor_rx.id,". implode(',',$joinTableColNames) ." FROM ".$sqlDB['table']." LEFT JOIN patient_profiles ON doctor_rx.patient_id=patient_profiles.id WHERE doctor_rx.user_id='".$user['id']."' AND doctor_rx.active IS ". $sqlDB['active'] ."";
